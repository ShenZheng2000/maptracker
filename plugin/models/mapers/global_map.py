...
# Ref: https://github.com/ShenZheng2000/Inf_Range_Perception/blob/171ff6d277f0c42ac36d75e2c276953bba8c06b0/projects/mmdet3d_plugin/hrmap/global_map.py
import numpy as np
import torch
...
class GlobalMap:
    def __init__(self, map_cfg):
        ...
        if dataset == 'av2':
            ...
            # NOTE: hardcode fix: expand bev_radius to avoid OOB error (but will increase memory usage)
            # bev_radius = bev_radius * 100
            ...
            # ✅ Update city_list to only include cities found in train AND val!!!!!!!!!
            self.city_list = sorted(set(self.train_min_lidar_loc) & set(self.val_min_lidar_loc))

            # print("=== auto-computed bounds (raw, no safety margin) ===")
            # print("city_list =", self.city_list)
        ...
        # TODO: compute auto bounds for nuscenes dataset
        else:
