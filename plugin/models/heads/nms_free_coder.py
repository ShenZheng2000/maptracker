...
from .ops.diff_ras.polygon import SoftPolygon
...
class MapTRNMSFreeCoder(BaseBBoxCoder):
    ...
    def decode_raster(self, preds_dicts):
        ...
        # # print all these shapes and val for debug
        # print("[NMSFreeCoder] all_cls_scores shape:", all_cls_scores.shape)
        # print("[NMSFreeCoder] all_bbox_preds shape:", all_bbox_preds.shape)
        # print("[NMSFreeCoder] all_pts_preds shape:", all_pts_preds.shape)

        # # Print some actual numbers (detach to CPU to avoid flooding GPU logs)
        # print("Example cls_scores[0, 0]:", all_cls_scores[0, 0].detach().cpu().numpy())
        # print("Example bbox_preds[0, 0]:", all_bbox_preds[0, 0].detach().cpu().numpy())
        # print("Example pts_preds[0, 0, :5]:", all_pts_preds[0, 0, :5].detach().cpu().numpy())
